...
public class RecyclerViewOnScrollListener extends RecyclerView.OnScrollListener {
      ...
      case RecyclerView.SCROLL_STATE_IDLE:// 当不滚动时
        ...
        if (mCurrentPos >= 0 && mPresenter.getTotalEntities() != null) {
          ...
          if (mCurrentPos != totalItemCount - 1) {
            // 防止刷新的时候crash
            GankDailyDataEntity entity = mPresenter.getTotalEntities().get(mCurrentPos);
            // 获取日期
  ...
  public void notifyBackgroundChange() {
    ...
    try {
      gankDailyDataEntity = totalEntities.get(mCurrentPos);
    } catch (Exception e) {
      e.printStackTrace();
      return;
    }
      ...
      Flowable.just(url)
          ...
          .flatMap(new Function<String, Publisher<File>>() {
            @Override public Publisher<File> apply(String s) throws Exception {
              return Flowable.just(GlideApp.with(mPresenter.getView().getViewContext())
                  .load(url)
